...
int main(int argc, char* argv[]) {
   ...
   for (int size = matrix_size_min; size <= matrix_size_max; size += stride) {
       ...
       //
       {
           std::unique_ptr<DataInitializerCUDA> data2 = std::make_unique<DataInitializerCUDA>(size, 0, size, 3);
       ...
       //
       {
           std::unique_ptr<DataInitializerCuDSS> data3 = std::make_unique<DataInitializerCuDSS>(size, 0, size, 3);
